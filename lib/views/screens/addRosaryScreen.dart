...
class AddRosaryScreen extends StatelessWidget{
              ...
              children: [
                ...
                IconButton(onPressed: (){
                  if(rosaryKey.currentState!.validate()){
                    provider.addNewRosary();
                    AppRouter.NavigateToWidget(RosaryScreen());
                    ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          backgroundColor:SettingScreen.isGreen?Color(0xFF344F24):Colors.black,
                          content: Text('تم اضافة التسبيحة',textAlign: TextAlign.center,style: GoogleFonts.reemKufi(fontSize: 20.sp)),
                        ));
                  }
                }, icon: Icon(Icons.done,size: 40,color: Colors.green,)),
              ],
            ),
            title: Text('اضافة تسبيحة',style: GoogleFonts.reemKufi(fontSize: 25.sp),),
            centerTitle: true,
            actions: [
              IconButton(onPressed: (){
                AppRouter.NavigateToWidget(RosaryScreen());
